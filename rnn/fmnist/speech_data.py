...
def maybe_download(_file, work_directory):
  """Download the data from Pannous's website, unless it's already here."""
  print("Looking for data %s in %s" % (_file, work_directory))
  if not os.path.exists(work_directory):
    os.mkdir(work_directory)
  filepath = os.path.join(work_directory, re.sub('.*\/', '', _file))
  if not os.path.exists(filepath):
    if not _file.startswith("http"): url_filename = SOURCE_URL + _file
    else: url_filename = _file
    ...
    print('Successfully downloaded', _file, statinfo.st_size, 'bytes.')
...
def speaker(_file):  # vom Dateinamen
  # if not "_" in file:
  #   return "Unknown"
  return _file.split("_")[1]

def get_speakers(path=pcm_path):
  files = os.listdir(path)
  def nobad(_file):
    return "_" in _file and not "." in _file.split("_")[1]
